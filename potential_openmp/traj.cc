...
kvec.resize(ksize);  
  ...
  //#pragma omp parallel for reduction(+:nk) schedule (dynamic) default(shared) 
  for(int i3=1; i3<=n_side; i3++){
      ...
      if (k2<=kcut2){
         kmod[nk]=std::sqrt(k2); //kmod.push_back(std::sqrt(k2));
         kvec[nk]=coordinates(kx,ky,kz);//kvec.push_back(coordinates(kx,ky,kz));
         nk += 1;
      }
    }
...
//int start= nk/numt * rank ; 
//int stop; 
//if(rank == numt-1) { stop = nk;} 
//else {stop = (nk/numt)*(rank+1) ;}  
...
{double arg, kx, ky, kz, coordx, coordy, coordz;// sum=0.;
std::vector<coordinates> centered_coords = center_coords();
//coordinates diff;
int nat =get_nat();
potential.resize(nat,0); 
precalc_kcoeffs();
#pragma omp parallel for schedule(dynamic) default(shared) private(arg, kx, ky, kz, coordx, coordy, coordz) 
for (int i=0; i<nat; i++) {
double sum=0.;
//#pragma omp parallel reduction(+:sum) default(shared) private(real)
//{ int numt = omp_get_num_threads();
  //int rank = omp_get_thread_num();
///int chunk_k = nk/numt; 
#pragma omp parallel for schedule (dynamic) reduction(+:sum) default(shared) private(arg, kx, ky, kz, coordx, coordy, coordz) 
