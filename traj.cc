...
centered_coords.push_back(diff);}
}}
...
void Kvectors::gen_kvec() {
    ...
    for(int i3=1; i3<=n_side; i3++){
      ...
      double k2 = std::pow(kx,2) + std::pow(ky,2) + std::pow(kz,2);
      if (k2<=kcut2){
...
//printf("%d number of kvecs generated", nk);
//cout<<"\n" <<"dim of moduli"<< kmod.size();
//cout<<"\n" <<"dim of vec"<< kvec.size();
}
double Kvectors::prefunc_k (double k) {
return std::exp(-0.5* std::pow(sg*k, 2)) / std::pow(k,2)*std::sin(k);
}

void Kvectors::precalc_kcoeffs() {
for(int i=0;i<nk; i++) 
{kcoeffs.push_back(prefunc_k(kmod[i])); 
}
}
